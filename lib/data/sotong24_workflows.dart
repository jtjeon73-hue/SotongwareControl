...
class Sotong24WorkflowCatalog {
  ...
  static Sotong24WorkflowStageDef _ebookStage(
    ...
    final meta =
        ...
        (
          ...
          next: '다음 단계로 진행',
          approval: order == 15 || order == 17 || order == 18,
        );
  ...
  static final _ebookMeta =
      ...
      >{
        ...
        'user_review': (
          purpose: '편집 내부 체크포인트(완성형 r1 전 자동 가능).',
          work: '구조·톤·사례 방향을 내부적으로 확인한다.',
          ai: '편집 체크리스트를 제시하고 자동 통과할 수 있다.',
          outputs: '편집 체크포인트 메모',
        ...
        'editorial_structure_review': (
          purpose: '목차·분량·챕터 구조를 편집 검토한다.',
          work: '구조 검수 보고서를 작성한다.',
          ai: '목차·분량 불일치를 점검한다.',
          outputs: '편집·구조 검토 보고서',
          quality: ['목차 일치', '챕터 누락 없음'],
          user: ['구조 방향 확인'],
          next: '편집 내부 체크포인트',
          approval: false,
        ),
        'cover_layout_design': (
          purpose: '표지·내지 디자인을 확정한다.',
          work: '표지 이미지·레이아웃 산출물을 준비한다.',
          ai: '표지 시안·레이아웃 가이드를 생성한다.',
          outputs: '표지·디자인 패키지',
          quality: ['표지 파일 존재', '가독성'],
          user: ['표지 방향 확인'],
          next: 'PDF/EPUB 빌드',
          approval: false,
        ),
        'format_build': (
          purpose: 'canonical PDF·EPUB를 생성한다.',
          work: 'publish/book.pdf·book.epub·manifest를 준비한다.',
          ai: '포맷 빌드·해시·검증을 수행한다.',
          outputs: 'publish/book.pdf, publish/book.epub',
          quality: ['PDF signature', 'EPUB zip 구조', 'SHA256'],
          user: ['빌드 결과 확인'],
          next: '가독·접근성 검사',
          approval: false,
        ),
        'reader_accessibility_test': (
          purpose: '기기별 열람·접근성을 검사한다.',
          work: 'PDF/EPUB 열람 검사 보고서를 작성한다.',
          ai: '모바일·태블릿 열람 체크를 수행한다.',
          outputs: '리더 검사 보고서',
          quality: ['PDF/EPUB 열람', '모바일 가독'],
          user: ['검사 결과 확인'],
          next: '완성형 r1 사용자 검토',
          approval: false,
        ),
        'package_user_review': (
          purpose: '완성형 r1(원고·표지·PDF·EPUB)을 사용자가 검토한다.',
          work: 'manifest 기준 미리보기·다운로드 후 승인/보완/보류.',
          ai: '완성형 패키지를 고정하고 사용자 게이트에서 정지한다.',
          outputs: 'publish/r1_package_manifest.json',
          quality: ['PDF/EPUB/cover/manifest', '품질 PASS'],
          user: ['승인', '보완 요청', '보류', '품질 보고서'],
          next: '최종 폴리시 또는 보완 루프',
          approval: true,
        ),
        'final_polish': (
          purpose: '최종 고도화·상품성을 검증한다.',
          work: '폴리시·판매 메타를 정리한다.',
          ai: '완성도·차단 요인을 점검한다.',
          outputs: '최종 폴리시 보고서',
          quality: ['완성도', 'blocker 정리'],
          user: ['폴리시 확인'],
          next: '최종 사용자 승인',
          approval: false,
        ),
        'final_user_approval': (
          purpose: '판매/공개 전 최종 사용자 승인을 받는다.',
          work: '최종본을 확인하고 승인한다.',
          ai: '자동 승인하지 않는다.',
          outputs: '최종 승인 기록',
          quality: ['승인 기록'],
        ...
        'publication_package': (
          purpose: 'release_ready 패키지를 완성한다(외부 게시 제외).',
          work: '출시 준비 패키지를 고정한다.',
          ai: '외부 판매 사이트 등록은 수행하지 않는다.',
          outputs: 'publication package',
          quality: ['패키지 완비', '외부 게시 금지'],
          user: ['패키지 확인'],
          next: '완료',
          approval: true,
        ),
